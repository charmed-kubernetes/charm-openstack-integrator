...
get_port_sec_enabled = patch_fixture(
    "charms.layer.openstack.BaseLBImpl" ".get_port_sec_enabled",
...
def test_create_new(impl, log_err, kv):
    ...
    lb.create()
    impl.create_secgrp.assert_called_with("openstack-integrator-1234-app-members")

    impl.find_secgrp.side_effect = None
    impl.find_secgrp.return_value = None
    with pytest.raises(openstack.OpenStackLBError):
        lb.create()
    openstack.log_err.assert_called_with("Unable to find default " "security group")
...
def test_create_recover(impl, kv):
    ...
    impl.list_listeners.return_value = [{"name": "openstack-integrator-1234-app"}]
    impl.list_pools.return_value = [{"name": "openstack-integrator-1234-app"}]
    impl.list_fips.return_value = [
...
def test_delete_loadbalancer(impl, kv):
    ...
    openstack.kv().unset.assert_called_with("created_lbs.openstack-integrator-1234-app")
...
def test_find_matching_sg_rule(impl):
    ...
    impl.list_sg_rules.return_value = [{"Port Range": "", "IP Range": "1.0.0.0/8"}]
    assert lb._find_matching_sg_rule("sg_id", lb.address, lb.port)

    impl.list_sg_rules.return_value = [
        {"Port Range": "81:90", "IP Range": ""},
        {"Port Range": "", "IP Range": "2.0.0.0/8"},
    ]
...
def test_find(impl, log_err):
    ...
    log_err.assert_called_with(
        "Multiple {} found: {}", "foo", "openstack-integrator-1234-app"
    )
...
def test_update_credentials(_normalize_creds, _save_creds, log_err):
    ...
    subprocess.run.side_effect = MockCalledProcessError(1, b"permission denied")
